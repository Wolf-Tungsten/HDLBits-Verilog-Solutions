...
static inline uint8_t ref(uint8_t x) {
    ...
    // HDL: f = (x[3] & ~x[1]) | (~x[3] & x[1] & x[2]);
    uint8_t nx1 = static_cast<uint8_t>(~x1 & 1u);
    uint8_t nx3 = static_cast<uint8_t>(~x3 & 1u);
    uint8_t t1 = static_cast<uint8_t>(x3 & nx1);
    uint8_t t2 = static_cast<uint8_t>(nx3 & x1 & x2);
    return static_cast<uint8_t>((t1 | t2) & 1u);
}
...
int main(int argc, char **argv) {
    ...
    auto dut = std::make_unique<Vdut_078>(ctx.get());
    ...
    std::cout << "[TB] dut_078 passed: 4-input logic on x[4:1]" << std::endl;
