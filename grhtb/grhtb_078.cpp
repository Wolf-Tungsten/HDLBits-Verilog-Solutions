...
static std::uint8_t ref(std::uint8_t x)
{
    ...
    return static_cast<std::uint8_t>((x3 & static_cast<std::uint8_t>(~x1 & 1u)) |
                                     (static_cast<std::uint8_t>(~x3 & 1u) & x1 & x2));
}
...
int main()
{
    ...
    std::cout << "[GrhTB] dut_078 passed: 4-input logic on x[4:1]\n";
    return EXIT_SUCCESS;
}
